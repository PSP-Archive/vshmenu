...
int menu_setup(void)
{
	...
	//umd mode
	{
		cnf.umdmode = limit( cnf.umdmode , 0 , 3);
	}
	item_str[TMENU_UMD_MODE]=(char *)(iso[cnf.umdmode]);


	//video name
	scePaf_strcpy(umd_path, (char *)GetVideoName());
	item_str[TMENU_UMD_VIDEO]=umd_path;

	//plugins
	item_str[TMENU_XMB_PLUGINS] = (char *)(enable_disable[cnf.plugvsh]);
	item_str[TMENU_GAME_PLUGINS]= (char *)(enable_disable[cnf.pluggame]);
	item_str[TMENU_POPS_PLUGINS]= (char *)(enable_disable[cnf.plugpop]);
